...
def ezville_loop(config):
        ...
        if device in RS485_DEVICE:
            ...
            if value == cur_state:
                ...
                if device == 'light':
                    ...
                    sendcmd = checksum(
                        'F7' + RS485_DEVICE[device]['power']['id'] + '1' + str(idx) + RS485_DEVICE[device]['power'][
                            'cmd'] + '030' + str(sid) + pwr + '000000')
                    ...
                    if value == 'OFF':
                        sendcmd = checksum(
                            'F7' + RS485_DEVICE[device]['power']['id'] + '0' + str(idx) + RS485_DEVICE[device]['power'][
                                'cmd'] + '0100' + '0000')
                        recvcmd = [
                            'F7' + RS485_DEVICE[device]['power']['id'] + '1' + str(idx) + RS485_DEVICE[device]['power'][
                                'ack']]
                    ...
                    CMD = '{:0>2X}'.format(
                        int('00' + eldown_state + elup_state + '0' + group_state + out_state + '0', 2))

                    # 일괄 차단기는 state를 변경하여 제공해서 월패드에서 조작하도록 해야함
                    # 월패드의 ACK는 무시
                    sendcmd = checksum(
                        'F7' + RS485_DEVICE[device]['state']['id'] + '0' + str(idx) + RS485_DEVICE[device]['state'][
                            'cmd'] + '0300' + CMD + '000000')
                    recvcmd = 'NULL'
                    statcmd = [key, 'NULL']
        ...
        for i in range(CMD_RETRY_COUNT):
            ...
            if comm_mode == 'mqtt':
                ...
                try:
                    ...
                    soc.sendall(bytes.fromhex(send_data['sendcmd']))
            if debug:
                log('[DEBUG] Iter. No.: ' + str(i + 1) + ', Target: ' + send_data['statcmd'][
                    1] + ', Current: ' + DEVICE_STATE.get(send_data['statcmd'][0]))
    ...
    async def ew11_health_loop():
        while True:
            ...
            if timestamp - last_received_time > EW11_TIMEOUT:
                log('[WARNING] {} {} {}초간 신호를 받지 못했습니다. ew11 기기를 재시작합니다.'.format(timestamp, last_received_time,
                                                                                 EW11_TIMEOUT))
                try:
                    await reset_EW11()
    ...
    mqtt_client =  mqtt.Client(mqtt_client.CallbackAPIVersion.VERSION1, 'mqtt-ezville')
